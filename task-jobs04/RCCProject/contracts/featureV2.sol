...
contract FeatureV2 is Initializable, UUPSUpgradeable, PausableUpgradeable, AccessControlUpgradeable{
    ...
    function v2initialize() public initializer {  //升级合约中不允许有构造函数
    ...
    function updatePool(uint256 _pid) internal {
        ...
        PoolInfo storage _pool = poolInfo[_pid];
        if (block.number <= _pool.lastRewordBlock || _pool.allTokenNum == 0) { 
            return;
        }
    ...
    function getStAmount(uint256 _pid) public view returns(uint256){
        // UserInfo storage _user = userInfo[_pid][msg.sender];
        return userInfo[_pid][msg.sender].stAmount;
    } 
